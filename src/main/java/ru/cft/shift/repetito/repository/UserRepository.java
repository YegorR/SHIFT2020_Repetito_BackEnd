...
public interface UserRepository extends JpaRepository<UserEntity, Long>, JpaSpecificationExecutor<UserEntity> {
    @Query("from UserEntity u where (u.about = :search or :search='null') and (u.course = :course or :course=0) and (u.faculty = :faculty or :faculty='null') and (u.degree = :degree or :degree ='null') and (u.isTeacher = :isTeacher or :isTeacher='false') and (:subject in (select a.id from UserEntity b join b.subjects a where b.id= u.id) or :subject=0)")
